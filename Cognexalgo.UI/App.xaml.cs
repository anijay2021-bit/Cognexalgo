    ...
    public partial class App : Application
    {
        ...
        private async void OnLoginSuccess()
        {
            try 
            {
                ...
                var cfg = (Microsoft.Extensions.Configuration.IConfiguration?)Application.Current.Resources["Configuration"];
                var connStr = cfg?.GetConnectionString("AlgoDatabase") ?? "";
                if (!string.IsNullOrWhiteSpace(connStr) && !connStr.StartsWith("Host=localhost"))
                {
                    try
                    {
